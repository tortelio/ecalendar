    ...
    %case maps:is_key(daylight, TimeZones) of
        %true ->
            ...
            %case (TempDtStart < time_rrule_date(RRS, TempDtStart)) and (TempDtStart >= time_rrule_date(RRD, TempDtStart)) of 
                %true ->
                    %DtStart = iso8601:subtract_time(TempDtStart, TosdHour, TosdMin, 0);
                %false ->
                    %DtStart = iso8601:subtract_time(TempDtStart, TossHour, TossMin, 0)
            %end,
            %case (TempDtEnd < time_rrule_date(RRS, TempDtEnd)) and (TempDtEnd >= time_rrule_date(RRD, TempDtEnd)) of 
                %true ->
                    %DtEnd = iso8601:subtract_time(TempDtEnd, TosdHour, TosdMin, 0);
                %false ->
                    %DtEnd = iso8601:subtract_time(TempDtEnd, TossHour, TossMin, 0)
            %end;
        %false ->
            %#{standard := StanDard} = TimeZones,
